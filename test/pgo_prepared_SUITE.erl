...
prepare_returns_oids(_Config) ->
    {ok, Name, OIDs} = pgo:prepare(
        "test_oids", "SELECT * FROM prepared_test WHERE id = $1 AND name = $2"
    ),
...
prepared_query_select(_Config) ->
    {ok, _, OIDs} = pgo:prepare(
        "q_select", "SELECT id, name, value FROM prepared_test WHERE id = $1"
    ),
...
prepared_query_insert(_Config) ->
    {ok, _, OIDs} = pgo:prepare(
        "q_insert", "INSERT INTO prepared_test (name, value) VALUES ($1, $2)"
    ),
...
prepared_query_with_params(_Config) ->
    {ok, _, OIDs} = pgo:prepare(
        "q_params", "SELECT name FROM prepared_test WHERE value > $1 ORDER BY value"
    ),
...
prepared_query_rows_as_maps(_Config) ->
    {ok, _, OIDs} = pgo:prepare(
        "q_maps", "SELECT id, name, value FROM prepared_test WHERE id = $1"
    ),
    Result = pgo:query_prepared("q_maps", [1], OIDs, #{
        decode_opts => [return_rows_as_maps, column_name_as_atom]
    }),
    ?assertMatch(
        #{command := select, rows := [#{id := 1, name := <<"alice">>, value := 10}]}, Result
    ).
...
auto_prepare_across_pool(_Config) ->
    ...
    {ok, _} = pgo_sup:start_child(multi_pool, #{
        pool_size => 5,
        port => 5432,
        database => "test",
        user => "test",
    ...
    Results = [
        pgo:query_prepared("auto_prep_test", [I], OIDs, #{pool => multi_pool})
     || I <- lists:seq(1, 20)
    ],

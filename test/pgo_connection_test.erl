...
tcp_closed_is_ignored_test() ->
    ?assertEqual(
        keep_state_and_data, pgo_connection:handle_event(info, {tcp_closed, some_port}, undefined)
    ).

ssl_closed_is_ignored_test() ->
    ?assertEqual(
        keep_state_and_data, pgo_connection:handle_event(info, {ssl_closed, some_port}, undefined)
    ).

...
cases() ->
    [
        transaction_returns_value,
        transaction_aborted_raises,
        exceptions,
        select,
        insert_update,
        text_types,
...
init_per_group(clear, Config) ->
    ...
    {ok, _} = pgo_sup:start_child(default, #{
        pool_size => 1,
        port => 5432,
        database => "test",
        user => "test",
...
set_parameter(_Config) ->
    ...
    ?assertMatch(
        ...
        pgo:query("SELECT current_setting('application_name')")
    ),
...
int4_range(_Config) ->
    ...
    ?assertMatch(
        #{command := insert},
        pgo:query(
            "insert into foo_range (id, some_range) values (2, $1)",
            [{{4, 12}, {true, false}}]
        )
    ),
    ?assertMatch(
        #{rows := [{2, {{4, 12}, {true, false}}}]},
    ...
    ?assertMatch(
        #{command := insert},
        pgo:query(
            "insert into foo_range (id, some_range) values (3, $1)",
            [{{4, unbound}, {true, false}}]
        )
    ),
    ?assertMatch(
        #{rows := [{3, {{4, unbound}, {true, false}}}]},
    ...
    ?assertMatch(
        #{command := insert},
        pgo:query(
            "insert into foo_range (id, some_range) values (4, $1)",
            [empty]
        )
    ),
    ?assertMatch(
...
ts_range(_Config) ->
    ?assertMatch(
        #{command := create},
        pgo:query(
            "create temporary table foo_ts_range (id integer primary key, some_range tsrange)"
        )
    ),
    ?assertMatch(
        #{command := insert},
        pgo:query("insert into foo_ts_range (id, some_range) values (1, $1)", [
            {{{2001, 1, 1}, {4, 10, 0}}, {{2001, 1, 1}, {5, 10, 0.0}}}
        ])
    ),
    ?assertMatch(
        #{rows := [{1, {{{{2001, 1, 1}, {4, 10, 0}}, {{2001, 1, 1}, {5, 10, 0}}}, {true, true}}}]},
...
tstz_range(_Config) ->
    ?assertMatch(
        #{rows := [{{{{{2019, 6, 22}, {8, 0, 0}}, {{2019, 6, 22}, {9, 0, 0}}}, {true, true}}}]},
        pgo:query(
            "SELECT tstzrange('2019-06-22 08:00:00.000+00','2019-06-22 09:00:00.000+00', '[]');"
        )
    ),

    ?assertMatch(
        #{rows := [{{{{{2019, 6, 22}, {8, 0, 0}}, {{2019, 6, 22}, {9, 0, 0}}}, {true, false}}}]},
        pgo:query(
            "SELECT tstzrange('2019-06-22 08:00:00.000+00','2019-06-22 09:00:00.000+00', '[)');"
        )
    ),
    ...
    ?assertMatch(
        #{rows := [{{{{{2019, 6, 22}, {8, 0, 0}}, infinity}, {true, true}}}]},
        pgo:query("SELECT tstzrange('2019-06-22 08:00:00.000+00','infinity', '[]');")
    ),

    ?assertMatch(
        #{rows := [{{{'-infinity', infinity}, {true, true}}}]},
        pgo:query("SELECT tstzrange('-infinity','infinity', '[]');")
    ),

    ?assertMatch(
        #{command := create},
        pgo:query(
            "create temporary table foo_tstz_range (id integer primary key, some_range tstzrange)"
        )
    ),
    ?assertMatch(
        #{command := insert},
        pgo:query("insert into foo_tstz_range (id, some_range) values (1, $1)", [
            {{{2001, 1, 1}, {4, 10, 0}}, {{2001, 1, 1}, {5, 10, 0.0}}}
        ])
    ),
    ?assertMatch(
    ...
    ?assertMatch(
        #{rows := [{{{{{2020, 1, 1}, {0, 0, 0}}, infinity}, {true, false}}}]},
        pgo:query(<<"SELECT tstzrange($1, $2, '[)');">>, [{{2020, 1, 1}, {0, 0, 0}}, infinity])
    ),

    ?assertMatch(
        #{rows := [{{{'-infinity', {{2020, 1, 1}, {0, 0, 0}}}, {true, false}}}]},
...
exceptions(_Config) ->
    ...
    ?assertMatch(
        "Error encoding type interval. Expected, {interval {{Hours::integer(), Minutes::integer(), Seconds::integer()}, Days::integer(), months::integer()}}. Got, none.",
        lists:flatten(pgo:format_error(Reason))
    ),
...
select(_Config) ->
    ...
    ?assertEqual(
        "parameters needed for query not equal to number of arguments 1 != 0",
        Module:format_error(Reason)
    ),
    {error, {Module1, Reason1}} = pgo:query("select $1", [1, 2]),
    ?assertEqual(
        "parameters needed for query not equal to number of arguments 1 != 2",
        Module1:format_error(Reason1)
    ),
...
insert_update(_Config) ->
    ...
    ?assertMatch(
        #{command := insert}, pgo:query("insert into foo (id, some_text) values (2, 'hello again')")
    ),
    ...
    ?assertMatch(
        #{rows := [{1, <<"hello world">>}, {2, <<"hello again">>}]},
    ...
    ?assertMatch(
        #{rows := [{<<"hello world">>, 1}, {<<"hello again">>, 2}]},
        pgo:query("select some_text, id from foo order by id asc")
    ),
    ?assertMatch(
        #{rows := [{<<"hello again">>}]}, pgo:query("select some_text from foo where id = 2")
    ),
...
text_types(_Config) ->
    ...
    ?assertMatch(
        #{command := select, rows := [{<<"foo">>}]}, pgo:query("select $1::text", [<<"foo">>])
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo         ">>}]}, pgo:query("select 'foo'::char(12)")
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo         ">>}]},
        pgo:query("select $1::char(12)", [<<"foo">>])
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo">>}]}, pgo:query("select 'foo'::varchar(12)")
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo">>}]},
        pgo:query("select $1::varchar(12)", [<<"foo">>])
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo">>}]}, pgo:query("select 'foobar'::char(3)")
    ),
    ?assertMatch(
        #{command := select, rows := [{<<"foo">>}]}, pgo:query("select $1::char(3)", [<<"foobar">>])
    ).
...
rows_as_maps(_Config) ->
    ...
    ?assertMatch(
        #{command := select, rows := [#{<<"id">> := 1, <<"some_text">> := <<"hello">>}]},
        pgo:query("select * from foo_1", [], #{decode_opts => [return_rows_as_maps]})
    ).

json_jsonb(_Config) ->
    #{command := create} = pgo:query(
        "create table tmp (id integer primary key, a_json json, b_json jsonb)"
    ),
    #{command := insert} = pgo:query(
        "insert into tmp (id, a_json, b_json) values ($1, $2, $3)",
        [3, #{<<"a">> => <<"foo">>}, #{<<"b">> => <<"bar">>}]
    ),
...
types(_Config) ->
    ?assertMatch(
        #{command := create},
        pgo:query(
            "create table types (id integer primary key, an_integer integer, a_bigint bigint, a_text text, a_uuid uuid, a_bytea bytea, a_real real)"
        )
    ),
    ?assertMatch(
        #{command := insert},
        pgo:query(
            "insert into types (id, an_integer, a_bigint, a_text, a_uuid, a_bytea, a_real) values (1, null, null, null, null, null, null)"
        )
    ),
    ?assertMatch(
        #{rows := [{1, null, null, null, null, null, null}]},
        pgo:query("select * from types where id = 1")
    ),
    ?assertMatch(
        #{command := insert},
        pgo:query(
            "insert into types (id, an_integer, a_bigint, a_text, a_uuid, a_bytea, a_real) values ($1, $2, $3, $4, $5, $6, $7)",
            [2, null, null, null, null, null, null]
        )
    ),
    ?assertMatch(
        #{rows := [{2, null, null, null, null, null, null}]},
    ...
    ?assertMatch(
        ...
        pgo:query(
            ...
            [
                ...
                null,
                null,
                <<236, 0, 0, 208, 169, 204, 65, 147, 163, 9, 107, 147, 233, 112, 253, 222>>,
                null,
                null
            ]
        )
    ...
    ?assertMatch(
        {10, 42, 1099511627776,
            <<"And in the end, the love you take is equal to the love you make">>, _UUID,
            <<"deadbeef">>, _Float},
        Row
    ),
    {10, 42, 1099511627776, <<"And in the end, the love you take is equal to the love you make">>,
        UUID, <<"deadbeef">>, Float} = Row,
    ...
    ?assertMatch(
        #{command := insert, rows := [{15}]},
        pgo:query(
            "insert into types (id, an_integer, a_bigint, a_text, a_uuid, a_bytea, a_real) values ($1, $2, $3, $4, $5, $6, $7) RETURNING id",
...
numerics(_Config) ->
    ...
    #{command := create} = pgo:query(
        "create table numeric_tmp (id integer primary key, a_int integer, a_num numeric,\n"
        "                                      b_num numeric(5,3))"
    ),
    ?assertMatch(
        #{command := insert},
        pgo:query("insert into numeric_tmp (id, a_int, a_num) values ($1, $2, $3)", [
            1,
            1,
            0.010000001
        ])
    ...
    ?assertMatch(
        #{rows := [{1.0}]},
        pgo:query("select a_int::numeric(2, 1) from numeric_tmp where id = 1", [])
    ),
    ?assertMatch(
        #{rows := [{1.00}]},
        pgo:query("select a_int::numeric(3, 1) from numeric_tmp where id = 1", [])
    ),
    ?assertMatch(
        #{rows := [{0.01}]},
        pgo:query("select a_num::numeric(10, 3) from numeric_tmp where id = 1", [])
    ),
    ?assertMatch(
        #{rows := [{0.01}]},
        pgo:query("select a_num::numeric(10, 3) from numeric_tmp where id = 1", [])
    ...
    ?assertMatch(
        #{rows := [{1.0e-32}]}, pgo:query("select a_num::numeric from numeric_tmp where id = 2", [])
    ).

hstore(_Config) ->
    #{command := create} = pgo:query("CREATE EXTENSION hstore"),
    #{command := create} = pgo:query(
        "create table hstore_tmp (id integer primary key, labels hstore)"
    ),

    ?assertMatch(
        #{command := insert},
        pgo:query("insert into hstore_tmp (id, labels) values ($1, $2)", [
...
records(_Config) ->
    ...
    ?assertMatch(
        #{rows := [{[{1, <<"2">>}]}]}, pgo:query("SELECT ARRAY[(1, '2')::composite1]", [])
    ),

    ?assertMatch(
        #{rows := [{{null, <<"2">>}}]}, pgo:query("SELECT $1::composite1", [{null, <<"2">>}])
    ),
...
polygon(_Config) ->
    ?assertMatch(
        #{rows := [{#{vertices := [#{x := 1.0, y := 1.0}, #{x := 2.0, y := 3.0}]}}]},
...
line_segment(_Config) ->
    ?assertMatch(
        #{rows := [{#{point1 := #{x := 1.0, y := 1.0}, point2 := #{x := 2.0, y := 3.0}}}]},
        pgo:query("SELECT $1::lseg", [
            #{
                point1 => #{x => 1, y => 1},
                point2 => #{x => 2, y => 3}
            }
...
arrays(_Config) ->
    ?assertMatch(
        #{rows := [{[<<"s1">>, null]}]}, pgo:query("SELECT $1::text[]", [[<<"s1">>, null]])
    ).

tsvector(_Config) ->
    ?assertMatch(
        #{rows := [{[{<<"fat">>, [{2, null}]}, {<<"rat">>, [{3, null}]}]}]},

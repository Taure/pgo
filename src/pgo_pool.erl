...
-type conn() :: #conn{}.
-type ref() :: {
    Pool :: pid(), Ref :: reference(), TimerRef :: reference() | undefined, Holder :: ets:tid()
}.
...
timeout(Delay, Time, Queue, Codel) ->
    case Codel of
        #{delay := MinDelay, next := Next, target := Target, interval := Interval} when
            Time >= Next andalso MinDelay > Target
        ->

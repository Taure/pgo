...
init([Pool, PoolConfig]) ->
    {ok, ready, #data{pool = Pool, pool_config = PoolConfig}, {next_event, internal, load}}.
...
-define(BOOTSTRAP_QUERY, [
    "SELECT t.oid, t.typname, t.typsend, t.typreceive, t.typlen, "
...
load_and_update_types(Conn, Pool) ->
    try
        case pgo_handler:simple_query(Conn, ?BOOTSTRAP_QUERY) of
            {ok, Oids} ->
                [
                 ...
                 || [Oid, Name, Send, Receive, Len, Output, Input, ArrayOid, BaseOid, CompOids] <-
                        Oids
                ];
            {error, {pgo_error, PgoError}} ->
                ?LOG_WARNING("Unable to load type information. query failed with error: ~p", [
                    PgoError
                ]),
                failed
        end
        ...
        Class:Reason:Stacktrace ->
            ?LOG_WARNING("Unable to load type information. pgo will not function properly: ~s", [

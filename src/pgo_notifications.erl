...
->
    try pgo_handler:open(undefined, Config) of
        ...
        {error, Error} ->
            ...
            {next_state, disconnected, Data#data{backoff = B1}, [{state_timeout, Backoff, connect}]}
    catch
        throw:Reason ->
            ?LOG_INFO(
                #{
...
disconnected(
    ...
    ListenerChannels1 = maps:update_with(
        Channel,
        fun(ChannelListeners) ->
...
connected(enter, _, #data{
    ...
    case map_size(ListenerChannels) > 0 of
        true ->
            ...
            ListenStatements = [
                ["LISTEN ", escape_channel_name(Channel), "; "]
             || Channel <- Channels
            ],
...
handle_event(
    cast, {set_parameter, Name, Value}, Data = #data{conn = Conn = #conn{parameters = Parameters}}
) ->
...
format_status(Status) ->
    maps:map(
        fun
            (data, Data = #data{config = PoolConfig}) ->
        ...
        {ok, {Channel, _Pid}} ->
            ...
            case erlang:map_size(PerChannel) of
                0 ->
                    ...
                    #{command := unlisten} = pgo_handler:extended_query(
                        Conn, ["UNLISTEN ", escape_channel_name(Channel)], []
                    ),
...
notify(ListenerChannels, #notification_response{
    channel = Channel,
    payload = Payload
}) ->
